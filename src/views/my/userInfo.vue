  ...
  <div class="">
    ...
    <div class="detail" :model="formData">
      ...
      <div class="clearfix rows">
        ...
        <div class="flr" style="height: .9rem;" @click="$router.push('/member')">
          <span class="go">立即认证</span>
          <i class="iconfont icon-xiangyou"></i>
        </div>
      </div>
      <div class="clearfix rows">
        <div class="fll clearfix">
          <i class="red fll">*</i> 姓名</div>
        <div class="flr">
          <input type="text" v-validate="'required|name'" name="姓名" v-model="formData.name" placeholder="请输入">
        </div>
        <span v-show="errors.has('姓名')" class="error langa flr">{{ errors.first('姓名')}}</span>
      </div>
      <div class="clearfix rows">
        <div class="fll clearfix">
          <i class="red fll">*</i> 联系电话</div>
        <div class="flr">
          <input type="text" v-validate="'required|mobile'" name="手机号码" v-model="formData.mobile" placeholder="请输入">
        </div>
        <span v-show="errors.has('手机号码')" class="error langa flr">{{ errors.first('手机号码')}}</span>
      </div>
      <div class="clearfix rows">
        <div class="fll clearfix"><i class="red fll">*</i> 电子邮箱</div>
        <div class="flr">
          <input type="text" v-validate="'required|email'" name="邮箱" v-model="formData.email" placeholder="请输入">
        </div>
        <span v-show="errors.has('邮箱')" class="error langa flr">{{ errors.first('邮箱')}}</span>
      </div>
      <div class="clearfix rows">
        <div class="fll">身份证号</div>
        <div class="flr">
          <input type="text" v-validate="'required|idcard'" name="身份证号码" v-model="formData.IDcard" placeholder="请输入">
        </div>
        <span v-show="errors.has('身份证号码')" class="error langa">{{ errors.first('身份证号码')}}</span>
      </div>

      <div class="clearfix rows">
        <div class="fll">性别</div>
        <div class="flr">
          <mu-radio v-model="formData.sex" class="itemRadio" value="1" label="男">男</mu-radio>
          <mu-radio v-model="formData.sex" class="itemRadio" value="2" label="女">女</mu-radio>
        </div>
      </div>
      <div class="clearfix rows">
        ...
        <span class="date-time-input" @click="getStartDate">
          <input type="text" class="dateYear flr" v-model="formData.birthdayTimeStr" placeholder="请选择">
        </span>
        <date-time ref="dateStartTime" type="date" @confirm="startSelect"></date-time>
        <!-- ---------- -->
      </div>
      <div class="clearfix rows">
        <div class="fll clearfix"><i class="red fll">*</i> 公司名称</div>
        <div class="flr">
          <input type="text" v-model="formData.company" placeholder="请输入">
        </div>
      </div>
      <div class="clearfix rows">
                <div class="fll clearfix"><i class="red fll">*</i> 职位</div>
        <div class="flr">
          <input type="text" v-model="formData.job" placeholder="请输入">
        </div>
      </div>
      <div class="clearfix rows">
                <div class="fll clearfix"><i class="red fll">*</i> 所属行业</div>
        <div class="flr">
          <select class="oneRows" v-model="formData.industrys">
            <option :value="item.dataValue" v-for="item in industryList" :key="item.dataValue" :label="item.dataName">{{item.dataName}}</option>
          </select>
        </div>
      </div>
      <div class="clearfix rows">
                <div class="fll clearfix"><i class="red fll">*</i> 所在地区</div>
        <i class="iconfont icon-xiangyou flr" @click="show = true"></i>
        <div @click="show = true">
          <span class="areaa flr">{{province}},{{city}},{{county}}</span>
        </div>
    ...
    <button class="subBtn" @click="submitForm">提交</button>
  </div>
...
<script>
  ...
  Validator.extend("idcard", {
    ...
    validate: value => {
      return (
        value.length == 18 && /(^\d{15}$)|(^\d{18}$)|(^\d{17}(\d|X|x)$)/.test(value)
      );
    }
  });
  Validator.extend("name", {
    getMessage: field => "请填写姓名",
    validate: value => {
      return (
        value.length != 0
      );
  ...
  Validator.extend("formData.email", {
    getMessage: field => "邮箱格式不正确",
    validate: value => {
      return;
      // value == /^[a-z0-9A-Z]+[- |a-z0-9A-Z . _]+@([a-z0-9A-Z]+(-[a-z0-9A-Z]+)?\\.)+[a-z]{2,}$/.test(value)
    }
  });
  export default {
    components: {
    ...
    methods: {
      ...
      startSelect(val) {
        this.formData.birthdayTimeStr = val
        console.log(val)
      },
      // 图片上传
      onRead(file) {
      ...
      getData() {
        this.$axios.get("/jsp/wap/center/ctrl/jsonUserInfo.jsp").then(res => {
          ...
          if (this.province && this.city && this.county == null) {
            this.descArea = false;
          }
        });
      },
      submitForm() {
        this.$validator.validateAll().then(result => {
          if (result) {
            ...
            this.$axios.get("/jsp/wap/center/do/doEditUser.jsp", {
              params: {
                headImgPath: this.formData.headImgPath,
                name: this.formData.name,
                sex: this.formData.sex,
                birthdayTimeStr: this.formData.birthdayTimeStr,
      ...
      onConfirmDate(e) {
        console.log(e);
        this.showDate = false;
        // this.year = ele[0].name;
        // this.day = ele[1].name;
        // this.month = ele[2].name;
      },
      onChangeDate() { }
    },
    created() {
...
<style scoped lang="scss">
  ...
  /deep/ .d-date-time .d-date-time-content .d-date-time-weeks-wrap {
    margin: 0
  }
